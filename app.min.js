...
require("./config/database");mongoose.set("useFindAndModify",!1);mongoose.Promise=global.Promise;mongoose.connect(mongoDburl,{useNewUrlParser:!0,useUnifiedTopology:!0}).then(a=>console.log("Connected")).catch(a=>console.log(a));app.use(express.static(path.join(__dirname,"public")));app.engine("handlebars",handlebars({defaultLayout:"home",helpers:{},partialsDir:path.join(__dirname,"views/layouts/partials"),handlebars:allowInsecurePrototypeAccess(Handlebars)}));app.use(upload());
app.set("view engine","handlebars");app.use(bodyParser.json());app.use(bodyParser.urlencoded({extended:!0}));app.use(session({secret:"tonyAtuoha",resave:!0,saveUninitialized:!0}));app.use(flash());app.use(passport.initialize());app.use(passport.session());app.use((a,b,c)=>{b.locals.success_msg=a.flash("success_msg");b.locals.error_msg=a.flash("error_msg");b.locals.error=a.flash("error");b.locals.loggedUser=a.user||null;c()});app.use(methodOverride("_method"));const home=require("./routes/home/home");
app.use("/",home);app.listen(port,()=>{console.log(`listening to port: ${port}`)});