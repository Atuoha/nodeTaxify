...
router.post("/create",(b,c)=>{const a=new Contact;a.user=b.session.user;a.subject=b.body.subject;a.message=b.body.message;a.save().then(d=>{b.flash("success_msg","Contact has been created successfully :)");c.redirect("/admin/contacts")}).catch(d=>console.log(d))});
router.post("/dummy",(b,c)=>{for(let a=0;a<b.body.number;a++){const d=new Contact;d.user=b.session.user;d.subject=faker.random.word();d.message=faker.lorem.sentence();d.save().then(e=>{b.flash("success_msg",`${b.body.number} dummy contacts has been created successfully :)`);c.redirect("/admin/contact")}).catch(e=>console.log(e))}});
