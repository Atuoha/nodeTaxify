...
router.post("/create",(b,c)=>{const a=new Testimony;a.content=b.body.content;a.user=b.user.id;a.save().then(d=>{b.flash("success_msg","Testimony has been created successfully :)");c.redirect("/admin/testimonies")}).catch(d=>console.log(d))});
router.post("/dummy",(b,c)=>{for(let a=0;a<b.body.number;a++){const d=new Testimony;d.content=faker.lorem.sentence();d.user=b.user.id;d.save().then(e=>{b.flash("success_msg",`${b.body.number} dummy testimonies successfully created  :)`);c.redirect("/admin/testimonies")}).catch(e=>console.log(e))}});
