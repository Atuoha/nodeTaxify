...
d.mv("./public/uploads/"+single_image,g=>{if(g)throw g;})});(new Post({title:b.body.title,sub:b.body.sub,status:b.body.status,category:b.body.category,file:a,multi_files:c,body:b.body.body,user:b.user.id,date:new Date})).save().then(d=>{b.flash("success_msg",`Post: ${d.title} has been created :)`);e.redirect("/admin/posts")}).catch(d=>console.log(d))}});
...
{""!==f&&"img_place.png"!==f&&fs.unlink("./public/uploads/"+f,h=>{h&&console.log(h)})}),a.multi_files="empty",a.multi_files=g):a.multi_files=d;a.title=b.body.title;a.sub=b.body.sub;a.status=b.body.status;a.category=b.body.category;a.body=b.body.body;a.file=c;a.user=b.user.id;a.save().then(f=>{b.flash("success_msg",`Post has successfully been updated. New title is ${f.title}`);e.redirect("/admin/posts")}).catch(f=>console.log(`Updating Error from: ${f}`))}).catch(a=>console.log(`Can't find post due to error from: ${a}`))});
