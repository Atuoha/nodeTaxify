...
router.get("/",(b,c)=>{Booking.find({user:"5fb26dd6794fc32960e640c3"}).where("status").equals("Active").populate("user").then(a=>{c.render("accounts/user/booking",{bookings:a})}).catch(a=>console.log(a))});router.get("/cancelled",(b,c)=>{Booking.find({user:b.user.id}).where("status").equals("Unactive").populate("user").then(a=>{c.render("accounts/user/booking/cancelled",{bookings:a})}).catch(a=>console.log(a))});
